...
beforeEach(() => {
  doughnutJSON = {
    ...
    permissions: {
      ...
      somethingElse: new Uint8Array([35, 231, 113, 42, 1])
    }
  };
...
describe("Payload Version 0", () => {
  ...
  it("check encoded values of expiry and notBefore", () => {
    ...
    const offset = 1 + 32 + 32; // offset to timestamps
  ...
  it("check encoded values of domain lengths", () => {
    const source = doughnutJSON;
    const encoded = payloadVersion.encode(source);
    const offset = 1 + 32 + 32 + 4; // offset to domain list

    // check first domain length (4 = 0x0004) is LE
    expect(encoded.slice(offset + 16, offset + 18)).toEqual(new Uint8Array([0x04, 0x00]));

    // check second domain length (5 = 0x0005) is LE
    expect(encoded.slice(offset + 34, offset + 36)).toEqual(new Uint8Array([0x05, 0x00]));
  });
